      ...
      <div className="mx-auto w-full max-w-7xl pb-8">
        <div className="grid grid-cols-1 gap-8 px-1 sm:grid-cols-2 sm:gap-9 sm:px-2 md:grid-cols-3 lg:grid-cols-4">
          {getNewArrivals(8).map((product) => (
            <article
              key={product.id}
              className="group relative overflow-hidden rounded-2xl border border-neutral-200 bg-white shadow-sm transition-all duration-300 hover:-translate-y-1 hover:shadow-lg dark:border-neutral-800 dark:bg-neutral-900"
            >
              {/* Badge */}
              ...
              <div className="p-3 sm:p-4 md:p-6">
                {/* Brand */}
                <p className="text-xs font-medium tracking-wide text-neutral-500 uppercase sm:text-sm dark:text-neutral-400">
                  {product.brand}
                </p>

                {/* Product name */}
                <h3 className="mt-1.5 line-clamp-2 text-sm font-semibold text-neutral-900 transition-colors group-hover:text-neutral-600 sm:mt-2 sm:text-base dark:text-white dark:group-hover:text-neutral-300">
                  {product.name}
                </h3>
                ...
                {/* Price section */}
                <div className="mt-2 flex flex-col items-center justify-between gap-2 sm:mt-3 sm:flex-col sm:gap-2 md:mt-4 md:flex-col md:gap-2 lg:flex-row lg:gap-0">
                  ...
                  {/* Stock indicator */}
                  <div
                    className={`rounded-full px-2 py-1 text-center text-xs sm:px-3 sm:py-1.5 sm:text-sm md:w-full lg:w-auto ${
                      product.stock > 20
